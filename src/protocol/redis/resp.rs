...
impl MessageMut {
    ...
    fn parse_inner(cursor: usize, src: &[u8]) -> Result<Option<MsgPack>, AsError> {
        ...
        match src[cursor] {
            ...
            RESP_BULK => {
                let csize = match btoi::<isize>(&src[cursor + 1..cursor + pos - 1]) {
                    Ok(csize) => csize,
...
impl Message {
        ...
        Message {
            ...
            resp_type: RespType::Array(
                ...
                vec![
                    ...
                    RespType::Bulk(
                        Range::new(14, 17 + len_len),
                        Range::new(17 + len_len, 17 + len_len + len + 2),
                    ),
                ],
            ),
        }
    ...
    pub fn inline_raw(data: Bytes) -> Message {
        ...
        Message {
            resp_type: RespType::Inline(rngs),
            data,
        }
    }
    ...
    pub fn save(&self, buf: &mut BytesMut) -> usize {
        self.save_by_resp_type(&self.resp_type, buf)
    }

    pub fn save_by_resp_type(&self, rtype: &RespType, buf: &mut BytesMut) -> usize {
        match rtype {
            RespType::String(rg) => {
            ...
            RespType::Array(head, subs) => {
                ...
                for sub in subs {
                    size += self.save_by_resp_type(sub, buf);
                }
                size
            }
    ...
    pub fn data(&self) -> Option<&[u8]> {
        let range = self.get_range(Some(&self.resp_type));
    ...
    fn get_nth_data_range(&self, index: usize) -> Option<Range> {
        if let RespType::Array(_, items) = &self.resp_type {
            return self.get_range(items.get(index));
        }
        if let RespType::Inline(fields) = &self.resp_type {
            if let Some(rng) = fields.get(index) {
                let mut end = rng.end();
                let len = rng.range();
    ...
    pub fn replace_info_resp(&mut self) {
        if let RespType::Bulk(_, body) = self.resp_type {
            if self.data.len() > 7 {
                ...
                let text = match std::str::from_utf8(data) {
                    ...
                    Err(err) => {
                        log::error!(
                            "replace_info_resp from_utf8 error: {}, data: {:?}",
                            err,
                            self.data
                        );
                        return;
                ...
                self.resp_type = RespType::Bulk(
                    Range::new(0, head_len),
                    Range::new(head_len, head_len + body_len),
                );
                self.data = data;
            }
        }
...
fn pattern_finder() -> &'static AhoCorasick {
    static FINDER: OnceLock<AhoCorasick> = OnceLock::new();
    FINDER.get_or_init(|| {
        AhoCorasick::new(PATTERNS).expect("failed to create a pattern matching aho-corasick")
    })
}

fn parse_redirect(data: &[u8]) -> Option<Redirect> {
    if let Some(mat) = pattern_finder().find(data) {
        let pat = mat.pattern();
        let end = mat.end();
    ...
    fn test_parse_inline() {
        ...
        assert!(
            msg == Message {
                ...
                resp_type: RespType::Inline(vec![Range::new(0, 3), Range::new(4, 6),]),
            }
        )
    }
