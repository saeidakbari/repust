...
static TEXT_CMD_FINDER: OnceLock<AhoCorasick> = OnceLock::new();

static TEXT_RESP_FINDER: OnceLock<AhoCorasick> = OnceLock::new();

pub fn init_text_finder() {
    let _ = TEXT_CMD_FINDER.set(
        AhoCorasickBuilder::new()
            .match_kind(MatchKind::LeftmostFirst)
            .build(TEXT_CMDS)
            .expect("fail to build text command finder"),
    );
    let _ = TEXT_RESP_FINDER.set(
        AhoCorasickBuilder::new()
            .match_kind(MatchKind::LeftmostFirst)
            .build(TEXT_RESPS)
            .expect("fail to build text response finder"),
    );
}
...
impl Message {
    pub fn parse(data: &mut BytesMut) -> Result<Option<Message>, AsError> {
        ...
        if let Some(mat) = TEXT_CMD_FINDER
            .get()
            .unwrap()
            .find(&data[..min(line_size, MSG_TEXT_MAX_CMD_SIZE)])
        {
            return Self::parse_text_req(data, line_size, mat.pattern().as_usize());
        }

        if let Some(mat) = TEXT_RESP_FINDER
            .get()
            .unwrap()
            .find(&data[..min(line_size, MSG_TEXT_MAX_RESP_TYPE_SIZE)])
        {
            return Self::parse_text_value(
                data,
                line_size,
                mat.pattern().as_usize() != TEXT_RESP_PAT_VALUE,
            );
        }
...
impl<'a> From<&'a AsError> for Message {
    fn from(val: &'a AsError) -> Self {
        Message {
...
mod test {
    ...
    use self::super::*;
    use crate::protocol::mc::msg::init_text_finder;
    ...
    fn test_parse_bin() {
        ...
        let items = vec![
            Message {
                data: Bytes::from(get_req_bin_data[..].to_owned()),
                flags: CmdFlags::empty(),
                mtype: MsgType::Binary {
                    btype: BinType::Req,
                    bmtype: BinMsgType::GetK,
            ...
            // next is for binary cases
            Message {
                data: Bytes::from(get_resp_bin_data[..].to_owned()),
                flags: CmdFlags::empty(),
                mtype: MsgType::Binary {
                    btype: BinType::Resp,
                    bmtype: BinMsgType::GetK,
    ...
    fn test_parser_error() {
        ...
        let mut data = BytesMut::from(&fuzz_data[..]);
